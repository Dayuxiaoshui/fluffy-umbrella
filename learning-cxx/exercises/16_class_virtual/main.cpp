...
int main(int argc, char **argv) {
    ...
    ASSERT(d.virtual_name() == 'C', MSG); // 因为虽然是 D，但是 C 的 final 特性，因此 D->C
    ...
    ASSERT(rac.virtual_name() == 'C', MSG); // 因为虽然是 D，但是 C 的 final 特性，因此 D->C
    ASSERT(rbd.virtual_name() == 'C', MSG); // 因为虽然是 D，但是 C 的 final 特性，因此 D->C
    ASSERT(rac.direct_name() == 'C', MSG);
    ASSERT(rbd.direct_name() == 'D', MSG);

    A &rad = d;

    ASSERT(rad.virtual_name() == 'C', MSG); // 因为虽然是 D，但是 C 的 final 特性，因此 D->C
