...
int main(int argc, char **argv) {
    ...
    ASSERT((std::is_same_v<decltype(hello), std::string>), "Fill in the missing type."); // std::string type check
    ASSERT((std::is_same_v<decltype(world), const char*>), "Fill in the missing type."); // const char* type check
    // TODO: 将 `?` 替换为正确的字符串
    ASSERT(hello + ", " + world + '!' == "Hello, world!", "Fill in the missing string."); // Corrected string comparison
    return 0;
}
